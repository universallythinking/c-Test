...
template <typename String>
static string decode(String encoded_string, bool remove_linebreaks) {
 //
 ...
 //
    ...
    while (pos < length_of_string) {
       ...
       ret.push_back(static_cast<string::value_type>( ( (pos_of_char(encoded_string[pos+0]) ) << 2 ) + ( (pos_of_char_1 & 0x30 ) >> 4)));
       ...
       if (encoded_string[pos+2] != '=' && encoded_string[pos+2] != '.') { // accept URL-safe base 64 strings, too, so check for '.' also.
          ...
          if (encoded_string[pos+3] != '=' && encoded_string[pos+3] != '.') {
             ret.push_back(static_cast<string::value_type>( ( (pos_of_char_2 & 0x03 ) << 6 ) + pos_of_char(encoded_string[pos+3])   ));
          }
       }
...
string base64_decode(string const& s, bool remove_linebreaks) {
...
string base64_encode(string_view s, bool url) {
   return encode(s, url);
}
...
string base64_decode(string_view s, bool remove_linebreaks) {
   return decode(s, remove_linebreaks);
}
